...
plot.coef.matrix <- function (phi, p, name = NULL) {
    ...
    L2 <- levelplot(as.matrix(f(B)),  at = seq( -max(max(abs(phi))), max(max(abs(phi))), length=101), col.regions = rgb.palette(100), 
                    colorkey = NULL, xlab = NULL, ylab = NULL, main = list(label = name, cex = 1), panel = function(...) {
                                                                               panel.levelplot(...)
                                                                               panel.abline(a = NULL, b = 1, h = seq(1.5, m * s + p * k.y + 0.5, by = 1), 
                                                                                            v = seq(1.5, by = 1, length = p * k.x + m * s), lwd = 0.5)
                    ...
                    scales = list(x = list(alternating = 1, labels = text, cex = 2, at = at, tck = c(0, 0)), y = list(alternating = 0, tck = c(0, 0))))
    return(L2)
}


#' Plot the linear coefficient matrix
plot.omega.matrix <- function (phi, p, name = NULL) {
  B <- phi
  if (nrow(B) == 1) {
    B <- matrix(B[, 1:ncol(B)], nrow = 1)
  }
  ...
  text <- c()
  for (i in 1:p) {
    text1 <- as.expression(bquote(bold(Omega)^(.(i))))
    text <- append(text, text1)
  }
  if (m > 0) {
    for (i in (p + 1):(p + s + 1)) {
      text1 <- as.expression(bquote(bold(Omega)^(.(i - p - s1))))
      text <- append(text, text1)
    }
  }
  f <- function(m) t(m)[, nrow(m):1]
...
#' Plot the linear coefficient matrix
plot.sigma.matrix <- function (phi, p, name = NULL) {
  B <- phi
  if (nrow(B) == 1) {
    B <- matrix(B[, 1:ncol(B)], nrow = 1)
  }
  ...
  text <- c()
  for (i in 1:p) {
    text1 <- as.expression(bquote(bold(Sigma)^(.(i))))
    text <- append(text, text1)
  }
  if (m > 0) {
    for (i in (p + 1):(p + s + 1)) {
      text1 <- as.expression(bquote(bold(Sigma)^(.(i - p - s1))))
      text <- append(text, text1)
    }
  }
  f <- function(m) t(m)[, nrow(m):1]
...
constant.sim.break <- function (nobs, cnst, sigma, brk = nobs+1) {
  ...
  data_y = data_y[1:nT, ]
  data_e = data_e[1:nT, ]
  lmsim <- list(series_y = data_y, noises = data_e)
}
...
ggm.sim.break <- function (nobs, px,  sigma, brk = nobs+1) {
  ...
  if (m > 1){
    ...
    for ( mm in 1:(m-1)){
      for (it in (brk[mm]):(brk[mm+1]-1) ) {
        sigma.temp = sigma.full[, (mm*p.x + 1):(mm*p.x+ p.x)]
        data_x[it, ] = rmvnorm(1, rep(0, p.x), sigma.temp)
      }
    }
  }
...
tbfl <- function(method, data_y, data_x = NULL, lambda.1.cv = NULL, lambda.2.cv = NULL, q = 1, max.iteration = 100, tol = 10^(-2), block.size = NULL, blocks=NULL, fixed_index=NULL, HBIC = FALSE, gamma.val = NULL){
  ...
  if(method == 'Constant'){
    ...
    return(list(cp.first = cp.first, beta.est = beta.est, cp.final = cp.final, beta.hat.list = beta.hat.list ))
  ...
  if(method == 'GGM'){
    ...
    return(list(cp.first = cp.first, beta.est = beta.est, cp.final = cp.final, phi.full = temp.first$theta.full,  beta.hat.list = beta.hat.list ))
  }
  ...
  # if (method == 'MvLR_fixed'|  method =='MLR_fixed'){
  #   if (is.null(data_x)){
  #     print("ERROR: empty predictor data!"); break;
  #   }
  #   if (is.null(fixed_index)){
  #     print("ERROR: empty index for unchanged coefficients! Use MvLR method instead!"); break;
  #   }
  #   p.y <- length(data_y[1,]); p.x.all <- length(data_x[1,]);
  #   #k: the number of variables which are piece-wise constants
  #   p.x <- p.x.all - length(fixed_index);
  #   nonfixed_index <- c(1:p.x.all)[-fixed_index]
  #   data_x.org <- data_x; data_y.org <- data_y
  #   data_x.temp <- data_x[,-fixed_index]; data_y.temp <- data_y
  #   ############# Tuning parameter ################
  #   if(is.null(lambda.1.cv)){
  #     lambda.1.max <- lambda_warm_up_lm(data_y.temp, data_x.temp, blocks, cv.index)$lambda_1_max
  #     if(blocks[2] <= (p.x + p.y) ){
  #       epsilon <-  10^(-3)
  #     }
  #     if(blocks[2] >= (p.x + p.y) ){
  ...
  #   ####################################################
  #   
  #   temp.first <- lm.first.step.blocks(data_y, data_x, lambda.1.cv, lambda.2.cv, max.iteration = max.iteration, tol = tol, 
  #                                      blocks , cv.index, fixed_index, nonfixed_index)
  #   cp.first <- temp.first$pts.list;
  #   cl.number <- length(cp.first);
  #   beta.est <- temp.first$beta.full
  #   beta.est.fixed <- temp.first$beta.fixed.est
  #   temp.first.all<- temp.first
  #   
  #   
  #   data_x.fixed <- data_x[, fixed_index]
  #   data_y.temp <- data_y - data_x.fixed%*%t(beta.est.fixed)
  #   ####################################################
  #   ########## second step       #######################
  #   ####################################################
  #   if(length(cp.first) > 0){
  #     temp.second<- lm.second.step.search(data_y.temp, data_x.temp, max.iteration = max.iteration, tol = tol, cp.first, beta.est, blocks )
  #     temp.second.all<- temp.second
  #     cp.final<- temp.second$cp.final;
  #   }else{
  #     cp.final <- c()
  #     print('no change points!')
  #   }
  #   return(list(cp.first = cp.first, beta.est = beta.est, cp.final =cp.final, beta.est.fixed = beta.est.fixed ))
  # }
  if(method == 'VAR'){
     p <- length(data_y[1,]);
...
lm.first.step.blocks <- function(data_y, data_x, lambda1, lambda2, max.iteration = max.iteration, tol = tol,  blocks, cv.index, fixed_index = NULL, nonfixed_index = NULL, HBIC = FALSE, gamma.val = NULL){
  ...
  # print('use l1 norm!')
  # jumps <- jumps.l1
  ignore_num <- min(6 , round(200/mean(blocks.size)))
  ...
  # if( length(unique(jumps)) <= 2 ){
  #   if(length(unique(jumps)) == 2){
  #     pts.sel <- blocks[which.max(jumps)]
  #   }else{
  #     pts.sel <- c();
  #   }
  #   
  ...
  if ( length(pts.sel) > 0){
    ...
    if(length(cp.final) > 5){
      if( length( unique(gap.temp) ) > 2){
        # fit <- kmeans(gap.temp, 2)
        # loc.new <- fit$cluster;
        # print(loc.new)
        # cl.reorder = c(1:2)[order(fit$centers)]
        # cl.number <- sum(loc.new == cl.reorder[2]) + 1
        fit <- kmeans(unique(gap.temp), 2)
        loc.new <- fit$cluster;
        cl.reorder = c(1:2)[order(fit$centers)]
        min.gap.val <- min(unique(gap.temp)[loc.new == cl.reorder[2]])
        cl.number <-   sum(gap.temp >= min.gap.val)  + 1
      ...
      if( length( unique(gap.temp) ) <= 2){
        max.gap.val <- max(unique(gap.temp))
        min.gap.val <- min(unique(gap.temp))
        # need adjustment!!!!!
        if(max.gap.val - min.gap.val <= 2*min(blocks.size)){
          cl.number <- length(gap.temp)  + 1
        }else{
          cl.number <- sum(gap.temp == max.gap.val)  + 1
        }
        cluster.pos <- sort(order(gap.temp,decreasing = TRUE)[1:(cl.number-1)])
        cluster.pos <- c(0, cluster.pos, length(cp.final))
        pts.list <-  vector("list", cl.number);
...
ggm.first.step.blocks <- function(data_y, data_x, lambda1, lambda2, max.iteration = max.iteration, tol = tol, blocks, cv.index, HBIC = FALSE, gamma.val = NULL){
  ...
  for (i in 1:kk){
    ...
    # phi.hat.full.all <- matrix(0, nrow = p.y, ncol = n.new*p.x.temp)
    # for(j in 1:p.y){
    #   cv <- rep(0, kk);
    #   data.y.temp <- as.matrix(data_y[, j])
    #   data.x.temp <- data_x[, -j]
    # 
    #   if (i == 1){
    #     test <- lm_break_fit_block(data.y.temp, data.x.temp, lambda.full[i, 1], lambda.full[i, 2], max_iteration = max.iteration, tol = tol, initial_phi =  0.0+matrix(0.0, p.y.temp, p.x.temp*n.new), blocks = blocks, cv.index)
    #     flag.full[i] <- test$flag;
    #   }
    #   if ( i > 1 ){
    #     initial.phi <- phi.final[[i-1]]
    ...
    #   }
    #   phi.hat.full <- test$phi.hat;
    #   phi.hat.full.all[j, ] <- phi.hat.full;
    #   phi.final[[i]] <- phi.hat.full.all;
    # 
    #   #forecast the time series based on the estimated matrix Phi (beta for the linear regression model)
    #   #and compute the forecast error
  ...
  while(BIC.diff > 0 & length(unique(jumps)) > 1 ){
    ...
    #   forecast.all.new[j.1, (blocks[1]):(blocks[2]-1)] <- pred.block(t(data.x.temp), matrix(phi.full.all.new[[1]][j.1, ], ncol = p.x.temp),
    #                                                                  blocks[1], p.x.temp, p.y.temp, blocks[2] - blocks[1]);
    # }  
    # for(i.1 in 2:n.new){
    #   if((i.1 %in% loc.block.full)){
    #     phi.full.all.new[[i.1]] <- phi.full.all.new.temp[[i.1]]
    ...
    #   for(j.1 in 1:p.y){
    #     data.x.temp <- data_x[, -j.1]
    #     forecast.all.new[j.1, (blocks[i.1]):(blocks[i.1+1]-1)] <- pred.block(t(data.x.temp), matrix(phi.full.all.new[[i.1]][j.1, ], ncol = p.x.temp),
    #                                                                          blocks[i.1], p.x.temp, p.y.temp, blocks[i.1+1] - blocks[i.1]);
    #   }
    # }
    ...
    residual <- t(data_y[(1:T), ]) - forecast.all.new;
    # print(phi.full.all.new)
    # print(phi.full.all.new.temp)
    # print(residual)
...
detection.check <- function(pts.final, brk, N){
  ...
  for(i in 1:(m-1)){
    ...
    for(j in 1:N){
      ...
      temp.2 <- pts.final.full.2[[j]]; l.2 <- length(temp); loc.2[j] <- temp.2[i];
    }
    # loc <-  loc[which(loc!=0)]
  ...
  return(list(pts.final.full.1 = pts.final.full.1,  detection = detection, freq = freq))
}
