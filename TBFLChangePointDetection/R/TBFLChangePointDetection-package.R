...
#' @useDynLib TBFLChangePointDetection, .registration = TRUE
...
#' @export
#' @examples
#' T <- 2*10^3; # number of observations/samples
#' p.y <- 1; # dimension of observed Y
#' p.x <- 20
#' brk <- c(floor(T/4), floor(2*T/4), floor(3*T/4), T+1)
#' m <- length(brk)
...
#' try <- lm.sim.break(nobs = T, px = p.x, phi = beta.full, sigma = e.sigma, sigma_x = 1, brk = brk)
...
tbfl <- function(method, data_y, data_x = NULL, lambda.1.cv = NULL, lambda.2.cv = NULL, q = 1,
  ...
  if(optimal.block == TRUE){
    ...
    if(!is.null(block.range)){
        ...
        if(method == 'GGM'){
            if(sqrt(T) > p.x){
                b_n.max <- ceiling(min(sqrt(T), T/20))
                # b_n.max <- ceiling(min(sqrt(T), T/20)/5)*5
            }else{
                b_n.max <- ceiling(min(sqrt(T)*log(p.x), T/20))
                # b_n.max <- ceiling(min(sqrt(T)*log(p.x), T/20)/5)*5
            }
            b_n.min <- floor(min(log(T)*log(p.x), T/20 ))
            # b_n.min <- floor(min(log(T)*log(p.x), T/20 )/5)*5
            ...
            if(sqrt(T) > p.x*p.y){
                ...
                b_n.max <- ceiling(min(sqrt(T)*log(p.x*p.y), T/20))
                # b_n.max <- ceiling(min(sqrt(T)*log(p.x*p.y), T/20)/5)*5
            }
            b_n.min <- floor(min(log(T)*log(p.x*p.y), T/20 ))
            # b_n.min <- floor(min(log(T)*log(p.x*p.y), T/20 )/5)*5
            print("maximum block size:")
            print(b_n.max)
            print("minimum block size:")
            print(b_n.min)
    ...
    for(j.2 in 1:n.method){
      ...
      blocks <- c(seq(1, b_n_bound*2+1 , b_n_bound),
                  seq(b_n_bound*2+block.size+1, T+1-2*b_n_bound, block.size),
      ...
      if(method == 'Constant'){
        ...
        if(refit){
          ...
          cp.full <- c(1, cp.final, T+1)
          m <- length(cp.final) + 1
          temp_beta <-  list(m)
          for(i in 1:m){
            # data_x_temp <- matrix(1, nrow = T)
          ...
          lambda.val.best <- BIC.threshold(method, beta.final.temp, p.y,
                                           length(c(cp.final, T+1)), c(cp.final, T+1), data_y , b_n = block.size)

          for(j in 1:length(c(cp.final, T+1))){
          ...
          lambda.val.best <- BIC.threshold(method, beta.final.temp, p.y, length(cp.final) + 1, c(cp.final, T+1),
      ...
      if(method == 'GGM'){
        ...
        if(refit){
          ...
          lambda.val.best <- BIC.threshold.ggm(beta.final.temp, p.x-1, length(cp.final) + 1, c(cp.final, T+1),
      ...
      if (method == 'MvLR' |  method =="MLR"){
        ...
        if(refit){
          ...
          if(method == 'MLR'){
            beta.final.temp <- matrix(c(do.call("cbind", beta.hat.list)), nrow= 1)
            print(block.size)
            lambda.val.best <- BIC.threshold(method, beta.final.temp, p.x, length(cp.final) + 1, c(cp.final, T+1),
    ...
    for(j.2 in 1:n.method){
      ...
      if(method == 'Constant'){
        ...
        log.det <- log(det(sigma.hat));
        HBIC.res <- log.det*T + 2*optimal.gamma.val*log(p.y)*count
        # BIC.res <- log.det*T + (count+3*(p.x-1))*log(T)
      ...
      if(method == 'MLR'){
        ...
        p.y <- 1
        HBIC.res <- log.det*T + 2*optimal.gamma.val*log(p.x)*count
      ...
      if(method == 'GGM'){
        ...
        for(jjj in 1:p.x){
          ...
          log.det <- log(sigma.hat);
          HBIC.res <- c(HBIC.res, log.det*T + 2*optimal.gamma.val*log(p.x-1)*count)
        }
      }
    ...
    if(is.null(block.size) && is.null(blocks) ){
      ...
      block.size = floor(log(T)*log(p.y*p.x));
      # blocks <- seq(1, T + 1, block.size);
      b_n_bound = 2*block.size  #block size for boundary
      blocks <- c(seq(1, b_n_bound*2+1 , b_n_bound),
                  seq(b_n_bound*2+block.size+1, T+1-2*b_n_bound, block.size),
    ...
    if(method == 'VAR'){
      ...
      if(refit){
        ...
        beta.final.temp <- as.matrix(do.call("cbind", beta.hat.list))
        print(dim(beta.final.temp))
        lambda.val.best <- BIC.threshold(method, beta.final.temp, p.x*q, length(cp.final) + 1, c(cp.final, T+1),
                                         data_y, data_x,  b_n = block.size, nlam = 20)
  ...
  if ( length(pts.sel) > 0){
    ...
    if(length(cp.final) > 5){
      if(length(unique(gap.temp)) > 1  ){
        ...
        if(median(blocks.size) <= sqrt(T)/4){
          cnst <- 2*4+1
        }else if(median(blocks.size) <= 2*sqrt(T)/4){
          cnst <- 2*3+1
        }else{
          cnst <- 2*2+1
        }
        ...
        if(median(blocks.size) <= sqrt(T)/4){
          cnst2 <- 7
        }else if(median(blocks.size) <= sqrt(T)/2){
          cnst2 <- 5
        }else{
          cnst2 <- 3
        }
    ...
    if(length(cp.final) > 5  ){
      if(length(unique(gap.temp)) > 1  ){
        ...
        if(median(blocks.size) <= sqrt(T)/4){
          ...
          cnst <- 2*3+1
        }else if(median(blocks.size) <= 3*sqrt(T)/4){
          cnst <- 2*2+1
        }else{
          cnst <- 2*2+1
        }
...
#' @param q the AR order
#' @param T the start time point for prediction
...
#' @param p the number of time series components
#' @param h the length of observation to predict
pred.var <- function(Y, phi, q, T, p, h = 1){
  concat.Y <- matrix(0,p,q+h); concat.Y[,1:q] <- Y[, (T-q):(T-1)];
  for ( j in 1:h){
    temp <- matrix(0,p,1);
    for (i in 1:q){temp <- temp +  phi[,((i-1)*p+1):(i*p)]%*%concat.Y[,q+j-i];}
...
#' @return lambda.val.best, the tuning parameter lambda selected by BIC.
#'
BIC.threshold.ggm <- function(beta.final, k, m.hat, brk, data_y, data_x = NULL, b_n = floor(sqrt(T)) , nlam = 20){
  brk.full <- c(1, brk)
  flag = 0
  p.y <- k+1
