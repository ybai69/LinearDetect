...
//[[Rcpp::export]]
mat lm_break_fit_block_new(mat data_y, mat data_x, double lambda, double lambda2, int max_iteration, double tol , NumericMatrix initial_phi, NumericVector blocks, NumericVector cv_index ){
...
List ggm_break_fit_block(NumericMatrix data_y, NumericMatrix data_x, double lambda, double lambda2, int max_iteration, double tol , NumericMatrix initial_phi, NumericVector blocks, NumericVector cv_index ){
    ...
    mat data_y_m(data_y.begin(), T, p_y);
    mat data_x_m(data_x.begin(), T, p_x);
    mat phi_hat_full_all (p_y, n_new*p_x_temp, fill::zeros);
	for(int j = 0; j < p_y ; j++){
		mat data_y_temp = data_y_m( span::all ,  j );
		mat data_x_temp (T, p_x-1, fill::zeros);
		for(int i = 0; i < p_y; i++){
			if( i < j ){
		...
		mat test = lm_break_fit_block_new(data_y_temp, data_x_temp, lambda, lambda2, max_iteration, tol, initial_phi, blocks, cv_index);
		mat phi_hat_full = test;	
